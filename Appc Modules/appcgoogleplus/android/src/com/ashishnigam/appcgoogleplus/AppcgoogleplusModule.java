 ...
 *
 */
package com.ashishnigam.appcgoogleplus;
...
@Kroll.module(name = "Appcgoogleplus", id = "com.ashishnigam.appcgoogleplus")
public class AppcgoogleplusModule extends KrollModule
		implements
	...
	private static final String LCAT = "AppcgoogleplusModule";
	private static final String TAG = "AppcgoogleplusModule";
	...
	private boolean checkPlayServices() {
		Activity thisActivity = TiApplication.getAppCurrentActivity();
		int resultCode = GooglePlayServicesUtil
				.isGooglePlayServicesAvailable(thisActivity);
		if (resultCode != ConnectionResult.SUCCESS) {
			if (GooglePlayServicesUtil.isUserRecoverableError(resultCode)) {
				GooglePlayServicesUtil.getErrorDialog(resultCode, thisActivity,
						0).show();
			} else {
			...
			Toast.makeText(thisActivity,
					"This device support play services.",
					Toast.LENGTH_LONG).show();
		}
	...
	public void onConnected(Bundle connectionHint) {
		Log.d(TAG, "Connected!");
		Logger.getLogger(TAG).info("onConnected user profile: " + "test");
		...
		// implement the interface ResultCallback<People.LoadPeopleResult>, then
		// only this will be resolved
		...
		if (Plus.PeopleApi.getCurrentPerson(mGoogleApiClient) != null) {
			...
			// JSONObject json = new JSONObject(currentPerson);
			Gson gson = new Gson();
			String personJson = gson.toJson(currentPerson);

			// Log.d(TAG, "currentPerson: "+json);
			String personName = currentPerson.getDisplayName();
			// String personPhoto = currentPerson.getImage().getUrl();
			// String personGooglePlusProfile = currentPerson.getUrl();
			Logger.getLogger(TAG).info(
					"onConnected user profile: " + personJson + " name= "
							+ personName);
			// Logger.getLogger(TAG).info("currentPerson json: " +personJson
			// +" name= "+personName);
		}
	...
	public void onResult(Activity activity, int thisRequestCode,
		...
		if (thisRequestCode == RC_SIGN_IN) {
			...
			if (resultCode == Activity.RESULT_OK) {
				...
				// with startIntentSenderForResult (launchIntentSenderForResult
				// in
			...
			if (resultCode == Activity.RESULT_OK) {
				...
				String personDetails = null;//doFetchUserProfile();
				handleSignInSuccess(mEmail, oneTimeToken, personDetails);
				// handleSignInSuccess(mEmail, oneTimeToken, personDetails);
			} else {
	...
	private void handleSignInSuccess(String email, String token,
		...
		// if((personDetails != null) && (personDetails != "")){
		// JSONObject personJson = new JSONObject(personDetails);
		// event.put("userProfile", personJson);
		// }
		String personDetailsL = doFetchUserProfile();
		event.put("userProfile", personDetailsL);
	...
	private void handleGooglePlayException(
		...
		Dialog dialog = GooglePlayServicesUtil.getErrorDialog(
				playEx.getConnectionStatusCode(), activity, -99); // -99
																	// temporary
																	// request
																	// code
		dialog.show();
	...
	private String doFetchUserProfile() {
		...
		if (!mGoogleApiClient.isConnected()) {
			...
			mGoogleApiClient.connect();
			Logger.getLogger(TAG).info(
					"Clinet not connected in doFetchUserProfMthod: ");
		} else {
			Logger.getLogger(TAG)
					.info("Clinet connected in doFetchUserProfElse: "
			...
			if (Plus.PeopleApi.getCurrentPerson(mGoogleApiClient) != null) {
				...
				// JSONObject json = new JSONObject(currentPerson);
				// Gson gson = new Gson();
				personJson = currentPerson.toString(); // gson.toJson(currentPerson);
	...
	private void fetchAccessToken() {
		...
		FetchUserTokenTask task = new FetchUserTokenTask(
				TiApplication.getAppCurrentActivity(), mEmail, scopeStr);
		task.execute();
	}
	...
	public void onResult(LoadPeopleResult arg0) {
		...
		//doFetchToken();
		Logger.getLogger(TAG).info("LoadPeopleResult arg0 personJson: " + arg0.toString());
		Activity thisActivity = TiApplication.getAppCurrentActivity();
		Toast.makeText(thisActivity,
				"LoadPeopleResult is also called.",
				Toast.LENGTH_LONG).show();
		
		// try {
		// String token = fetchToken();
		// String personJson = doFetchUserProfile();
		// Logger.getLogger(TAG).info("Found personJson: " + personJson);
		// if (token != null) {
		// Logger.getLogger(TAG).info(
		// "Found token for email: " + mEmail + " - " + token);
		// handleSignInSuccess(mEmail, token, personJson);
		// }
		// } catch (IOException e) {
	...
	public class FetchUserTokenTask extends AsyncTask {
		...
		protected Object doInBackground(Object... arg0) {
			try {
				String token = fetchToken();
				String personJson = null;//doFetchUserProfile();
				//Logger.getLogger(TAG).info("Found personJson: " + personJson);
				if (token != null) { //(personJson != null)
					Logger.getLogger(TAG).info(
							"Found token for email: " + mEmail + " - " + token);
					handleSignInSuccess(mEmail, token, personJson);
				}
		...
		protected String fetchToken() throws IOException {
			try {
				...
				handleRecoverableException(userRecoverableException.getIntent());
