...
#import "ComAshishnigamAppcgoogleplusModule.h"
#import "TiBase.h"
#import "TiHost.h"
#import "TiUtils.h"
...
@implementation ComAshishnigamAppcgoogleplusModule
{
    NSError *errorDesc;
}
...
-(id)moduleGUID
{
	return @"dcbb981b-72d4-473f-8664-0bfe988a4cee";
}

// this is generated for your module, please do not change it
-(NSString*)moduleId
{
	return @"com.ashishnigam.appcgoogleplus";
}
...
- (void)finishedWithAuth: (GTMOAuth2Authentication *)auth
    ...
    if (error) {
        ...
        [event setValue:[NSString stringWithFormat:@"Err1= %@",[auth errorString]] forKey:@"error1"];
        [event setValue:[NSString stringWithFormat:@"Err2= %@",errorDesc] forKey:@"error2"];
        [event setValue:[NSString stringWithFormat:@"Err3= %@",error] forKey:@"error3"];
        if (errorCallback) {
            NSLog(@"[INFO] Firing Cancel Event.");
            [self _fireEventToListener:@"error" withObject:event listener:errorCallback
                            thisObject:nil];
        }
    } else {
        loggedIn = YES;
        ...
        [event setValue:[[[GPPSignIn sharedInstance] googlePlusUser] JSON] forKey:@"userProfile"];
        if (successCallback)
        {
...
-(void)signin:(id)args
{
    ...
    NSLog(@"[INFO] Values passed=%@ %@",clientId,scopeArr);
    if ([gps hasAuthInKeychain]) {
        GTMOAuth2Authentication *auth = [gps authentication];
        [self finishedWithAuth:auth error:errorDesc];
    } else {
